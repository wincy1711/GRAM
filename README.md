...
`D = 128`, `K = 2`, `T = 2`, `N_sup = 4`, trained for 40 epochs on 5.9 K
examples. Both arms use the same backbone, the same data and the same budget,
differing only in `guidance` — 0.43 M parameters for GRAM against 0.23 M for
the deterministic ablation, the difference being the four guidance MLPs. Each
`best.pt` is then evaluated 5 times independently on the 53-instance test
split. The *scale* is nothing like the paper's; the *qualitative behaviour* is
